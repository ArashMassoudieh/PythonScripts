...
# --- safe field coercion ---------------------------------------------------
# QGIS returns a NULL QVariant (not Python None) for empty fields. A bare
# "if v is not None" check passes for a null QVariant, so "v * 3.28" then raises
# "unsupported operand type(s) for *: 'QVariant' and 'float'". These helpers
# coerce any value (real None, null QVariant, or a number) to float/int or a
# default, so every field read below is null-safe.
...
# Basin name = the site id (last component of SITE_DIR), e.g. "AZ12-100". This
# makes the "Basin: <name>" header and the output <name>.basin filename track
# the site set in the session, so the multi-site rollout needs no per-site edit.
import os as _os
...
CRLF = "\r\n"
def blk(lines): return CRLF.join(lines) + CRLF   # block ends with End: + CRLF
...
# --- element name maps (HMS names are strings; build from ids) -------------
...
# ==========================================================================
# TOPOLOGY: read the single source of truth produced by build_topology.py.
# write_basin does NO topology derivation -- it looks up each element's
# downstream name from topology.gpkg. An element absent from the table was
# pruned (internal reach / collapsed junction) and is NOT written.
# ==========================================================================
topo_p = os.path.join(OUT_DIR, "topology.gpkg")
if not os.path.isfile(topo_p):
    raise Exception("topology.gpkg not found -- run build_topology.py before "
                    "write_basin.py")
topo = QgsVectorLayer(topo_p + "|layername=topology", "topology", "ogr")
if not topo.isValid():
    raise Exception("invalid topology layer: " + topo_p)

# name -> downstream HMS element name; and the set of names actually in the model
DS_NAME   = {}     # element name -> ds_name
IN_MODEL  = set()  # element names present in topology (others were pruned)
TOPO_NOTE = {}     # element name -> audit note
...
# convenience sets by type for the write loops
...
for f in subs.getFeatures():
    ...
    if sub_name(sid) not in SUB_IN:
        continue                       # not in topology (shouldn't happen)
    area_mi2 = num(f["area_km2"], 0.0) * 1e6 * M2_TO_MI2
    cx, cy = num(f["centroid_x"], 0.0), num(f["centroid_y"], 0.0)
    lat, lon = latlon(cx, cy)
    loss = (f["loss_method"] if "loss_method" in sub_flds and f["loss_method"]
            else DEF_LOSS)
    trans = (f["transform_method"] if "transform_method" in sub_flds and f["transform_method"]
             else DEF_TRANSFORM)
    # subbasins drain to their downstream element per topology.gpkg
    ...
    if trans == "SCS":
        ...
        lines += [
            ...
            ""]
    # baseflow (match reference default)
    lines += [
...
# ---- reaches ----
# need per-reach up/down canvas coords; use geometry endpoints oriented by z
def line_ends(geom):
    if geom.isMultipart():
...
for f in reaches.getFeatures():
    ...
    if rch_name(rid) not in RCH_IN:
        continue                       # pruned (internal stub) -- not in topology
    ...
    route = (f["route_method"] if "route_method" in rch_flds and f["route_method"]
             else DEF_ROUTE)
...
for f in juncs.getFeatures():
    ...
    if jct_name(jid) not in JCT_IN:
        continue                       # collapsed junction -- not in topology
...
# the sink is needed if anything in the topology drains to it
if SINK_NAME in DS_NAME.values():
    sink_needed = True
# locate the outlet reach for sink placement
outlet_reach = None
for f in reaches.getFeatures():
    if f["ds_type"] == "outlet":
        outlet_reach = f
...
# ---- sink (outlet) ----
if sink_needed:
    # place sink at the outlet reach downstream end if available, else (0,0)
    sx, sy = 0.0, 0.0
...
out.append(blk([
    ...
    "     Hms Schematic: Yes",
    "     Map Visible: No",
    "     Gridlines Visible: No",
    "     Flow Direction Visible: No",
    "End:"]))

# --- write with CRLF, no extra trailing blank handling needed --------------
with open(basin_out, "w", newline="") as fh:
    # blocks already CRLF-terminated; separate blocks with a blank CRLF line
